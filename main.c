      ...
      if(MyStateRegister > 0)
	{
	  if((MyStateRegister & (TIMESTAMP_CAPTURED)) == (TIMESTAMP_CAPTURED))
	      ...
	      if(ReadSPIEEPROMaddr != SPIEEPROMaddr)
		{
		  ...
		  TimeDateRegS.SPIAddress[0] = (ReadSPIEEPROMaddr >> 8) & 0xFF;
		  TimeDateRegS.SPIAddress[1] = ReadSPIEEPROMaddr & 0xFF;
  ...
  if((TimeDateRegS.SPIAddress[0] & (WEL)) == (WEL))
    {
      ...
      TimeDateRegS.SPIAddress[0] = (SPIEEPROMaddr >> 8) & 0xFF;
      TimeDateRegS.SPIAddress[1] = SPIEEPROMaddr & 0xFF;
      if(pagebarrier == 0)
	{
      ...
      else
	{
	  ...
	  Write_SPI(PtrTDSPICR, 1);
	  TimeDateRegS.SPICommand = WRITE;
	  TimeDateRegS.SPIAddress[0] = ((SPIEEPROMaddr + TIME_LENGTH) >> 8) & 0xFF;
	  TimeDateRegS.SPIAddress[1] = (SPIEEPROMaddr + TIME_LENGTH) & 0xFF;
